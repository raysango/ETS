namespace :patient_data do
  task create: :environment do
    ...
    admission.medications.create(name: 'Acetaminophen', dosage: 500, order_frequency_id: order_frequency_4.id, necessity: 'relieve pain')
    admission.medications.create(name: 'Naproxen', dosage: 500, order_frequency_id: order_frequency_6.id, necessity: 'relieve
swelling')
    admission.treatments.create(description: 'temporary bracing the right leg', necessity: 'restrict the motion')
    diagnostic_procedure = admission.diagnostic_procedures.create(description: 'an exploratory radiography', moment: Date.today - 88.hours)
